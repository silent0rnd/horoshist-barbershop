...
export default function GallerySection() {
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === 'ArrowRight') { event.preventDefault(); setActiveIndex((index) => index === null ? null : (index + 1) % GALLERY.length); }
      if (event.key === 'ArrowLeft') { event.preventDefault(); setActiveIndex((index) => index === null ? null : (index - 1 + GALLERY.length) % GALLERY.length); }
      if (event.key === 'Tab') {
        const controls = [...document.querySelectorAll<HTMLButtonElement>('.lightbox button')];
    ...
    <section id="gallery" className="section gallery-section" aria-labelledby="gallery-title">
      ...
      <div className="gallery-grid">
        {GALLERY.map((item, index) => (
          <motion.button key={item.id} type="button" className={`gallery-tile gallery-${item.size}`} onClick={(event) => { triggerRef.current = event.currentTarget; setActiveIndex(index); }} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.12 }} transition={{ duration: 0.55, delay: (index % 4) * 0.07 }} aria-label={`Открыть фото: ${item.caption}`}>
      ...
      <AnimatePresence>
        {active && activeIndex !== null && (
          <motion.div className="lightbox" role="dialog" aria-modal="true" aria-label={`Фото ${activeIndex + 1} из ${GALLERY.length}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setActiveIndex(null)}>
            <button ref={closeButtonRef} className="lightbox-close" type="button" onClick={() => setActiveIndex(null)} aria-label="Закрыть фото"><X /></button>
